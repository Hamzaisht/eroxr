-- Fix the security definer view issue by setting views to use SECURITY INVOKER
-- This ensures views respect RLS policies and use the calling user's permissions

-- Drop and recreate views with SECURITY INVOKER option
DROP VIEW IF EXISTS public.top_creators_by_earnings CASCADE;
CREATE VIEW public.top_creators_by_earnings 
...
-- Drop and recreate user_bookmarks view with SECURITY INVOKER
DROP VIEW IF EXISTS public.user_bookmarks CASCADE;
CREATE VIEW public.user_bookmarks 
WITH (security_invoker=on) AS
SELECT 
    ps.user_id,
    p.id,
