...
CREATE TABLE IF NOT EXISTS view_tracking (
  ...
  -- Add hour_bucket column for unique constraint
  hour_bucket TIMESTAMP WITH TIME ZONE GENERATED ALWAYS AS (DATE_TRUNC('hour', viewed_at)) STORED
);

-- Create unique constraint to prevent duplicate views within same hour
CREATE UNIQUE INDEX IF NOT EXISTS idx_view_tracking_unique_hourly 
ON view_tracking(content_id, content_type, viewer_fingerprint, hour_bucket);
