...
export const HeroContent = () => {
  ...
  return (
    <motion.div 
      className={`flex-1 text-center lg:text-left px-4 md:px-6 max-w-7xl mx-auto ${isMobile ? 'pt-20' : 'pt-32'}`}
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5 }}
    >
      <motion.h1 
        className={`${isMobile ? 'text-3xl' : isTablet ? 'text-4xl' : 'text-6xl'} font-bold mb-4 md:mb-6 bg-gradient-to-r from-white via-luxury-primary to-luxury-accent bg-clip-text text-transparent`}
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      <motion.p 
        className={`${isMobile ? 'text-base' : 'text-lg md:text-xl'} text-luxury-neutral mb-6 md:mb-8 max-w-2xl mx-auto lg:mx-0`}
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
